# -*- coding: utf-8 -*-
"""
Radio Sources de Vie - Sermon du soir (20:00)
Génère un long sermon du soir (15+ minutes) avec Gemini (voix Aoede).
...
Sujet différent du sermon du matin grâce à un état distinct.
...
- TOUT en français — zéro mot anglais, zéro créole
- Commencer EXACTEMENT par : "Chers auditeurs de Radio Sources de Vie, bienvenue à ce moment de la Parole de Dieu en cette soirée. Que la grâce et la paix de notre Seigneur Jésus-Christ soient avec vous."
- Minimum 500 mots
- Salutation chaleureuse en fin de journée
- Présenter le thème du soir et le contexte du verset principal
- Lire le verset {verset} en français (Louis Segond)
- Donner le contexte biblique du verset
- Préparer le terrain pour les trois points qui vont suivre
- Ton pastoral, chaleureux, profond, posé (ambiance du soir)
- Texte parlé fluide, pas de titres, pas de numérotation
- NE PAS développer les points — c'est l'introduction
...
def section_point(sujet: str, verset: str, numero: int, focus: str) -> str:
    ...
    prompt = f"""Tu es un pasteur prêchant un sermon radiophonique du SOIR sur "Radio Sources de Vie".
...
- Minimum 550 mots
- Commencer par une transition proche de : "{transition}"
- Développer ce point en profondeur
- Citer 2 ou 3 versets bibliques d'appui (Louis Segond)
- Donner une illustration concrète (situation vécue par la communauté haïtienne, en Haïti ou diaspora)
- Application pratique pour la vie quotidienne
- Ton pastoral, profond, encourageant
- Texte parlé fluide, pas de titres ni numérotation visible
...
def section_conclusion_et_appel(sujet: str, verset: str) -> str:
    prompt = f"""Tu es un pasteur terminant un sermon radiophonique du SOIR sur "Radio Sources de Vie".
...
- TOUT en français
- Minimum 600 mots
- Transition : "Mes bien-aimés, en concluant ce message du soir..."
- Résumer brièvement les trois points
...
- Bénédiction pour la nuit
- Encouragement pour le repos en Dieu
- Conclure par : "Que la paix du Seigneur Jésus vous accompagne cette nuit. Bonne soirée sur Radio Sources de Vie."
- Ton pastoral, plein de paix
- Texte parlé fluide
