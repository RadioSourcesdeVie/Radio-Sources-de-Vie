# -*- coding: utf-8 -*-
"""
Radio Sources de Vie - Sermon du matin (09:30)
Génère un long sermon du matin (15+ minutes) avec Gemini (voix Aoede).
Structure : Introduction, 3 points principaux, Conclusion, Appel à l'autel.
Rotation des sujets — jamais le même deux jours d'affilée (state/sermon_matin_state.json).
Sortie : sermon/sermon_matin_AAAAMMJJ_HHMMSS.wav
Usage  : python sermon_matin.py
"""
import sys
...
Tu commences un long sermon radiophonique du matin sur le thème : "{sujet}"
...
- Commencer EXACTEMENT par : "Chers auditeurs de Radio Sources de Vie, bienvenue à ce moment de la Parole de Dieu. Que la grâce et la paix de notre Seigneur Jésus-Christ soient avec vous en ce matin béni."
- Minimum 500 mots
...
def section_point(sujet: str, verset: str, numero: int, focus: str) -> str:
    ...
    prompt = f"""Tu es un pasteur prêchant un sermon radiophonique sur "Radio Sources de Vie".

Thème du sermon : "{sujet}" (verset principal : {verset})
...
- Donner une illustration concrète (exemple de vie, situation que vit la communauté haïtienne)
- Application pratique pour la vie quotidienne (en Haïti et dans la diaspora)
...
def section_conclusion_et_appel(sujet: str, verset: str) -> str:
    prompt = f"""Tu es un pasteur terminant un sermon radiophonique sur "Radio Sources de Vie".
...
- APPEL À L'AUTEL chaleureux et sincère :
  ...
  * Inviter ceux qui se sont éloignés à revenir au Seigneur
  * Inviter ceux qui souffrent à confier leur fardeau au Christ
  * Conduire une prière de salut simple et sincère
- Bénédiction finale
- Encouragement pour la journée
- Conclure par : "Que la grâce du Seigneur Jésus soit avec vous. Restez à l'écoute de Radio Sources de Vie."
...
def main():
    index = get_topic_index('sermon_matin_state.json', len(SUJETS))
    ...
    titre = f"Sermon du matin — {sujet} ({verset})"
    full_script, tts_text = build_full_script(texte_complet, titre)

    base_nom = f"sermon_matin_{timestamp}"
    ...
    print(f"\nSermon du matin généré avec succès!")
    print(f"Sujet  : {sujet}")
    print(f"Verset : {verset}")
