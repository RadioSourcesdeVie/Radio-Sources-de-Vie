# -*- coding: utf-8 -*-
"""
Kit Radio IA - Sermon du matin (09:30 par défaut)
...
- Commencer EXACTEMENT par : "Chers auditeurs de {config.STATION_NOM}, bienvenue à ce moment de la Parole de Dieu. Que la grâce et la paix de notre Seigneur Jésus-Christ soient avec vous en ce matin béni."
- Minimum 500 mots
...
def section_point(sujet: str, verset: str, numero: int, focus: str) -> str:
    ...
    prompt = f"""Tu es un pasteur prêchant un sermon radiophonique sur "{config.STATION_NOM}".
...
def section_conclusion_et_appel(sujet: str, verset: str) -> str:
    prompt = f"""Tu es un pasteur terminant un sermon radiophonique sur "{config.STATION_NOM}".
...
def main():
    ...
    print(f"\n=== {config.STATION_NOM} — Sermon du matin ===")
    print(f"Sujet  : {sujet}")
