# -*- coding: utf-8 -*-
"""
Kit Radio IA - Prière du soir (21:00 par défaut)
Génère une longue prière du soir (15+ minutes) avec Gemini.
...
SUJETS = [
    f"Prière du soir pour la paix de {config.PAYS_OU_COMMUNAUTE}",
...
if config.NOM_DIASPORA:
    SUJETS.insert(5, f"Prière du soir pour {config.NOM_DIASPORA}")

OUTPUT_DIR = BASE_DIR / "priere"
OUTPUT_DIR.mkdir(exist_ok=True)

_DIASPORA_LIGNE = f", et pour {config.NOM_DIASPORA}" if config.NOM_DIASPORA else ""
...
- Commencer EXACTEMENT par : "Bienvenue à notre moment de prière du soir sur {config.STATION_NOM}. En cette fin de journée, recueillons-nous ensemble devant notre Père céleste pour Lui confier tout ce que nous avons vécu."
- Minimum 600 mots
...
def section_pardon(sujet: str) -> str:
    prompt = f"""Tu es un pasteur de "{config.STATION_NOM}".
...
- Intercession pour {config.PAYS_OU_COMMUNAUTE} : la paix, la sécurité, les enfants qui dorment, les familles
- Intercession pour{_DIASPORA_LIGNE if _DIASPORA_LIGNE else " ceux qui sont loin des leurs"}
...
- Conclure par : "Bonne nuit à tous nos auditeurs. Que Dieu vous garde et vous bénisse. Au revoir, sur {config.STATION_NOM}."
...
def main():
    ...
    print(f"\n=== {config.STATION_NOM} — Prière du soir ===")
    print(f"Sujet : {sujet}")
