# -*- coding: utf-8 -*-
"""
Kit Radio IA - Segment Témoignages (18:00 par défaut)
Génère un long segment de témoignages communautaires (10+ minutes) avec Gemini.
...
THEMES = [
    ...
    "le courage d'une famille qui persévère dans la foi loin des siens",
    ...
    "la fidélité de Dieu pour les jeunes qui cherchent leur voie",
]

OUTPUT_DIR = BASE_DIR / "temoignage"
OUTPUT_DIR.mkdir(exist_ok=True)

_ORIGINE = config.PAYS_OU_COMMUNAUTE
_DIASPORA = f" ou de {config.NOM_DIASPORA}" if config.NOM_DIASPORA else ""


def section_introduction(theme: str) -> str:
    prompt = f"""Tu es animateur chaleureux de "{config.STATION_NOM}", radio chrétienne francophone.

Génère l'INTRODUCTION du segment Témoignages communautaires sur le thème : {theme}

INSTRUCTIONS STRICTES :
- TOUT en {config.STATION_LANGUE}
- Commencer EXACTEMENT par : "Bienvenue à notre segment de témoignages sur {config.STATION_NOM}. Ce précieux moment est dédié à tous nos auditeurs qui partagent comment Dieu a été fidèle dans leur vie."
- Minimum 350 mots
...
def section_temoignage_1(theme: str) -> str:
    prompt = f"""Tu es animateur de "{config.STATION_NOM}".
...
- Inventer un témoin crédible de la communauté ({_ORIGINE}{_DIASPORA}) avec un prénom courant de cette communauté
...
- Inventer un témoin différent du premier, avec un prénom courant de la communauté ({_ORIGINE}{_DIASPORA})
...
- Inventer un troisième témoin distinct avec un prénom courant de la communauté
...
- Après le témoignage, faire la CLÔTURE du segment :
  ...
  * Invitation chaleureuse à partager son propre témoignage avec {config.STATION_NOM}
  * Verset biblique de clôture (Apocalypse 12:11 ou Psaume 66:16)
  * Bénédiction finale
  * Conclure par : "Merci d'avoir partagé ce moment avec nous. Restez à l'écoute de {config.STATION_NOM}."
- Pas de titres ni numérotation visible
- Texte parlé fluide
...
def main():
    ...
    print(f"\n=== {config.STATION_NOM} — Témoignages ===")
    print(f"Thème : {theme}")
